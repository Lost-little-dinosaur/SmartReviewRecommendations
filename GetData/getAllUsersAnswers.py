    ...
    for eachQuestionSet in questionSetIDArr:
        ...
        for each in userIDArr:
            if len(each) != 0:
                ...
                if count > 10:  # 是大于10条答题记录的用户，不是答题个数大于10的用户（可能有多选题）
        ...
        # print(userIDArr)
        # print(len(userIDArr))
        # 将paper转成ID对应有用数据的dict
        valueableValuePaperDict = {}
        for each in dfPapers.values:
            valueableValuePaperDict[each[0]] = [each[1], each[10], each[11]]  # 【开始时间，题库ID，生成类型】
        userAnswerArr = []
        for each in userIDArr:
            ...
            for eachAnswer in tempArr:
                ...
                if eachAnswer[2] not in tempDict:
                    tempDict[eachAnswer[2]] = [[eachAnswer[3], tempTime, valueableValuePaperDict[eachAnswer[1]][2]]]
                else:
                    tempDict[eachAnswer[2]].append([eachAnswer[3], tempTime, valueableValuePaperDict[eachAnswer[1]][2]])
