...
x_data = np.array([0, 5, 20, 60, 540, 1440, 8640, 44640])
y_data = np.array([1.00000, 0.84111, 0.78053, 0.68016, 0.63800, 0.57856, 0.54982, 0.40860])

# 使用curve_fit进行非线性拟合
params, _ = curve_fit(power_law_decay, x_data, y_data)

# 输出拟合参数
print("a:", params[0], "b:", params[1], "c:", params[2])
plt.figure()
plt.plot(x_data, y_data, 'b-', label='data')
plt.plot(x_data, power_law_decay(x_data, params[0], params[1], params[2]), 'r-', label='fit')
plt.legend()
plt.show()
...
#                         plt.clf()
#                     except:
#                         print('Error - could not clear current figure')
#                 else:
